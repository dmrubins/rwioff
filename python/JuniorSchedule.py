...
class JuniorSchedule(Schedule):
    ...
    def _get_shift_from_block(self, block, prev_shift):
        ...
        if block.is_("GMS") or block.is_("BMT") or block.is_("CHF") or block.is_("OFF") or block.is_("hem"):
            if isWeekend:
                return day_off
            else:
        ...
        if block.is_("vacation") or block.is_("Holiday") or block.is_("ED") or block.is_("Day off giver") or block.is_("hol-vac") or block.is_("DPH"):
            return day_off
        ...
        print("Junior Get shift failed; Block: {}, Date: {}".format(block.get_summary(), block.dt))
        input("")
